"""
Train RT-DETR-X on the stenosis detection dataset.
...
def convert_coco_val_to_yolo():
    ...
    print("[setup] Converting COCO val annotations → YOLO format …")
...
def compute_coco_metrics() -> dict:
    ...
    Matches the exact metric suite reported in arXiv:2503.01601 (Table I):
      - mAP@[0.50:0.95], mAP50, mAP75
      - mAP (small / medium / large)
      - AR @ maxDets=100 / 300 / 1000
      - AR (small / medium / large)

    Uses pycocotools directly so AR@300 and AR@1000 are available
    (Ultralytics only reports AR@100 by default).
    """
    try:
    ...
    # Map image filename → COCO image_id
    fname_to_id = {
        pathlib.Path(img["file_name"]).name: img["id"]
        for img in coco_gt.dataset["images"]
    }

    # stenosis category_id in the GT JSON
    stenosis_cat = next(
    ...
    preds = model.predict(
        source=VAL_IMAGES,
        imgsz=IMG_SIZE,
        conf=0.001,   # low threshold so pycocotools can sweep all scores
        iou=0.65,
        verbose=False,
    )
    ...
    # ── Pass 1: standard COCO stats + AR@100 ──────────────────────────────────
    ...
    # ── Pass 2: AR@300 (MMDetection default, used in paper) ───────────────────
    ...
    # ── Pass 3: AR@1000 (MMDetection default, used in paper) ──────────────────
...
def main():
    ...
    # 5. Full COCO evaluation + summary
    print_summary(start)
