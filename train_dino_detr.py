"""
Train DINO-DETR (5-scale, ResNet-50 backbone) on the stenosis detection dataset.
Uses MMDetection v3.x — mirrors the paper (arXiv:2503.01601) exactly.

Config used by the paper: dino-5scale_r50_4xb2-12e_coco.py
...
IMG_SIZE    = 800          # paper default for DINO-DETR
EPOCHS      = 12           # 1x COCO schedule (paper)
...
def build_config():
    """
    Generate a custom mmdet config extending the paper's base DINO-DETR config
    ...
    base_cfg = mmdet_root / ".mim" / "configs" / "dino" / "dino-5scale_r50_4xb2-12e_coco.py"
    if not base_cfg.exists():
        base_cfg = mmdet_root / "configs" / "dino" / "dino-5scale_r50_4xb2-12e_coco.py"
    if not base_cfg.exists():
        raise FileNotFoundError(
            f"Base config not found in either:\n"
            f"  {mmdet_root / '.mim' / 'configs' / 'dino'}\n"
            f"  {mmdet_root / 'configs' / 'dino'}\n"
