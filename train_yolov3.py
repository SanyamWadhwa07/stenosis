"""
Train YOLOv3 (DarkNet-53 backbone) on the stenosis detection dataset.
Uses MMDetection v3.x — mirrors the paper (arXiv:2503.01601) exactly.

Config used by the paper: yolov3_d53_8xb8-ms-608-273e_coco.py
...
def _resolve_base_cfg(mmdet_root: pathlib.Path, subdir: str, filename: str) -> pathlib.Path:
    ...
    # 2. Recursive search within the mmdet package tree
...
# ─── Config ───────────────────────────────────────────────────────────────────
MODEL_NAME  = "yolov3-d53"
IMG_SIZE    = 608          # paper uses multi-scale around 608
EPOCHS      = 273          # paper schedule (use fewer for quick experiments)
BATCH       = 8            # paper uses 8xb8; adapt to 1 GPU
WORKERS     = 4
SEED        = 42
DEVICE      = "cuda:0"
...
def filter_coco_to_stenosis(src: str, dst: str):
    """
    Write a filtered COCO JSON that contains only the 'stenosis' category
...
def build_config():
    """
    Generate a custom mmdet config that extends the paper's base config
    and points all paths at the ARCADE stenosis dataset.
...
# ── Training schedule ─────────────────────────────────────────────────────────
train_cfg = dict(max_epochs={EPOCHS}, val_interval=10)
...
def compute_coco_metrics() -> dict:
    ...
    inferencer = DetInferencer(model=cfg_path, weights=checkpoint, device=DEVICE)
